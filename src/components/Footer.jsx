...
const ICONS = {
  ...
  zalo: 'M24 4C12.95 4 4 12.95 4 24c0 6.1 2.7 11.5 7 15.2V44l4.6-2.5c1.3.4 2.7.5 4.1.5h.3c11.05 0 20-8.95 20-20S35.05 4 24 4zm2.2 26.9l-5.1-5.4-9.9 5.5 10.9-11.6 5.2 5.4 9.8-5.4-10.9 11.5z',
};

export default function Footer() {
  const { categories, settings: s } = useAdmin();
  const parentCats = categories.filter(c => !c.parent_id);

  const phone = s.phone1 || '0398.945.409';
  const socials = [
    s.facebook_url && { name: 'Facebook', href: s.facebook_url, icon: ICONS.facebook },
    s.tiktok_url && { name: 'TikTok', href: s.tiktok_url, icon: ICONS.tiktok },
    s.zalo_phone && { name: 'Zalo', href: `https://zalo.me/${s.zalo_phone}`, icon: ICONS.zalo },
  ].filter(Boolean);
    ...
    <footer>
      ...
      <div className="bg-slate-900 text-white/55">
        <div className="max-w-[1200px] mx-auto px-4 py-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1.5fr] gap-8 lg:gap-10">
          <div>
            <img src="/logoo.jpg" alt="Logo" className="h-10 w-auto object-contain mb-3" />
            ...
            <div className="text-sm flex flex-wrap items-center gap-x-4 gap-y-1.5">
              <span className="flex items-center gap-1.5">
                ...
                {s.address || '158 Nguyễn Ảnh Thủ, Thới Tam Thôn, Hóc Môn'}
              </span>
              ...
              <span className="text-white/20">|</span>
              <span className="text-white/40">{s.working_hours || 'T2 – CN: 8h00 – 21h00'}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="list-none flex flex-col gap-2.5">
              {parentCats.map(c => (
                <li key={c.id}>
                  <Link to={`/?category=${c.slug}`} className="text-white/50 no-underline text-sm hover:text-white transition-colors">{c.name}</Link>
                </li>
              ))}
            </ul>
          </div>
